...
pais_exacto = "Brussels, Belgium"
G = ox.graph_from_place(pais_exacto, network_type="drive")

for edge in G.edges:
    # Limpiar la variable de Velocida dmáxima
    vel_maxima = 40
    if "vel_maxima" in G.edges[edge]:
        vel_maxima = G.edges[edge]["vel_maxima"]
        if type(vel_maxima) == list:
            speeds = [ int(speed) for speed in vel_maxima ]
            vel_maxima = min(speeds)
        elif type(vel_maxima) == str:
            vel_maxima = int(vel_maxima)
    G.edges[edge]["vel_maxima"] = vel_maxima
    # Formula de peso = Longitud/Velocidad máxima
    G.edges[edge]["weight"] = G.edges[edge]["length"] / vel_maxima
...
def reconstruct_path(orig, dest, plot=False, algorithm=None):
    ...
    while curr != orig:
        ...
        speeds.append(G.edges[(prev, curr, 0)]["vel_maxima"])
